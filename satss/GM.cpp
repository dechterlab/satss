...
void GM::convertToSAT()
{
	...
	for(int i=0;i<variables.size();i++)
	{
		...
		/*num_con+=combination(variables[i]->num_values,2);
		if(variables[i]->num_values >1)
		num_con++;*/
	}
	for(int i=0;i<variables.size();i++)
	{
...
void GM::rearrangeOrdering(std::vector<int> &order, std::vector<set<int> > &clusters, std::vector<int> &new_order, double& limit)
{
	vector<vector<int> > var2clusters (variables.size());
	cout<<"Rearranging order\n";
	...
	while(!faulting_clusters.empty())
	{
		...
		double div=(double)variables[min_id]->domain_size();
		...
		for(int i=0;i<var2clusters[min_id].size();i++)
		{
			...
			{			
			        cluster2estimate[cluster_id]-=log(div);
...
{
	gm.variables=vector<Variable*>();
	for(int i=0;i<variables.size();i++){
		gm.variables.push_back(new Variable(i,variables[i]->domain()));
		if (variables[i]->value()!=INVALID_VALUE){
			gm.variables[i]->value()=variables[i]->value();
		}
	}
	cerr<<"Added other gm's variables\n";
	gm.functions=vector<Function*>(functions.size());
	for(int i=0;i<functions.size();i++){
		CPT* cpt=static_cast<CPT*> (functions[i]);
		vector<Variable*> curr_func_variables;
		for(int j=0;j<functions[i]->variables().size();j++){
			curr_func_variables.push_back(gm.variables[functions[i]->variables()[j]->id()]);
		}
		Variable* curr_marg_variable=gm.variables[cpt->marg_variable()->id()];
		assert(curr_marg_variable->id()==i);
		gm.functions[i]=new CPT(i,curr_func_variables,curr_marg_variable);
