...
RBSampleSearch::RBSampleSearch(): interval(10),outfilename("out")
{
}
...
void RBSampleSearch::reduce(GM &gm, std::vector<vector<bool> >& new_domains)
{
	...
	for(int i=0;i<new_domains.size();i++)
	{
		for(int j=0;j<new_domains[i].size();j++)
		{
			...
			/*vec<lbool>& assignment=solver.model;
			for(int a=0;a<new_domains.size();a++)
				for(int b=0;b<new_domains[a].size();b++)
					if(assignment[gm.csp_to_sat_variables[a][b]]==l_True)
					{
						new_domains[a][b]=true;
					}*/
		}
	}
	cerr<<"Domains reduced\n";
}
Double RBSampleSearch::computePE(GM& gm, JG& jg, int p_bound, vector<int>& order,vector<int>& sampling_order, int num_samples)
{
	gm.convertToSAT();
	...
	for(int i=0;i<num_samples;i++)
	{
		...
		for(int j=0;j<sampling_order.size();j++)
		{
			...
			{
				norm_const+=func_table[k];
			}
			for(int k=0;k<func_table.size();k++){
			  func_table[k]/=norm_const;
			}
			int value=INVALID_VALUE;
			double rand_num=random.getDouble();
			double cdf=0.0;
			...
			for(int k=0;k<func_table.size();k++)
			{
				...
				{
					value=k;
					cdf+=func_table[k].value();
					curr_var_weight=func_table[k].value();
					if(rand_num <= cdf)
					{
						value=k;
						break;
			...
			sample_weight+=log(curr_var_weight);
		}
		...
		Double pe=weight/Double((double)i+1);
		pe*=gm.mult_factor;
...
pair<long double,long double> purgeORStore(GM& gm, IJGPSamplingHelper& helper, ORStore& store, vector<int>& sampling_order, vector<vector<int> >& all_samples, vector<long double>& all_weights,ofstream& out)
{
	...
	for(int i=0;i<all_samples.size();i++){
		...
		//cout<<"OR store: "<<exp(all_weights[i]-sample_weight_lb)<<endl;
...
void purgeORStoreMarginals(GM& gm, 
						   ...
						   vector<vector<Double> >& marginals_lb,
						   vector<vector<Double> >& marginals_ub, ofstream& out)
{
...
void updateMarginals(GM& gm, GM& other_gm, vector<vector<Double> >& marginals,myRandom& random)
{
	...
	/*for(int i=0;i<gm.variables.size();i++){
		int var=gm.variables[i]->orig_id;
		assert((int)gm.variables[i]->old_domain.size()==(int)other_gm.variables[var]->domain_size());
	}*/
...
void printMarginals(GM& gm, 
					GM& other_gm,
					vector<vector<Double> >& marginals_lb,
					vector<vector<Double> >& marginals_ub, 
					vector<vector<Double> >& other_marginals,
					ofstream& out)
{
...
void purgeORStoreBayesMarginals(GM& gm, 
							GM& other_gm,
						   IJGPSamplingHelper& helper, 
						   ORStore& store, 
						   vector<int>& sampling_order, 
						   ...
						   ofstream& out)
{
	...
	for(int i=0;i<all_samples.size();i++){
		...
		//cout<<exp(all_weights[i]-sample_weight_lb)*gm.mult_factor.value()<<endl;
		// Now that all variables are set update the marginals
		for(int j=0;j<gm.variables.size();j++){
			assert(gm.variables[j]->value()!=INVALID_VALUE);
			marginals_lb[j][gm.variables[j]->value()]+=Double(exp(all_weights[i]-sample_weight_lb));
...
Double RBSampleSearch::computePEApp(GM& gm, JG& jg, int p_bound, vector<int>& order,vector<int>& sampling_order, int num_samples, int max_restarts)
{
...
//	
	...
	for(int i=0;i<num_samples;i++)
	{
		...
		if (i > 1 && i % interval == 0){
			...
			//weight+=(exp(actual_weight-sample_weight));
			//cout<<tmp.first<<" "<<tmp.second<<"  "<<gm.mult_factor<<endl;
			long double pe_lb=(weight_lb+tmp.first)/(long double)(i+1);
			
			pe_lb*=gm.mult_factor.value();
			//Double pe_ub=Double((weight_ub+tmp.second)/(long double)(i+1));
			long double pe_ub=(weight_ub+tmp.second)/(long double)(i+1);
			...
			cout<<"z <"<<log10(pe_lb)<<","<< log10(pe_ub)<<" > "<<endl;
			cout<<"z <"<<pe_lb<<","<< pe_ub<<" > "<<endl;
	...
	//return Double(weight_ub)/Double((long double)num_samples);
}
void RBSampleSearch::computeBeliefsApp(GM& gm, JG& jg, int p_bound, vector<int>& order,vector<int>& sampling_order, int num_samples, int max_restarts)
{
	// Initialize the SAT solver
	ofstream out(outfilename.c_str());
	out<<"t "<<getDateAndTime()<<endl;
...
//	
	...
	for(int i=0;i<num_samples;i++)
	{
		...
		if (i > 1 && i % interval == 0){
			purgeORStoreMarginals(gm,helper,store,sampling_order,rem_order,all_samples,rem_samples,all_weights,marginals_lb,marginals_ub,out);
			//purgeORStore(gm, helper, store, sampling_order, all_samples, all_weights,out);
			//weight_lb+=tmp.first;
			//weight_ub+=tmp.second;
			//weight+=(exp(actual_weight-sample_weight));
			//cout<<tmp.first<<" "<<tmp.second<<"  "<<gm.mult_factor<<endl;
...
void RBSampleSearch::computeBayesBeliefsApp(GM& gm, GM& other_gm, JG& jg, int p_bound, vector<int>& order,vector<int>& sampling_order, int num_samples, int max_restarts)
{
	// Initialize the SAT solver
	cout<<"Starting Bayes Belief updating\n";
	ofstream out(outfilename.c_str());
	out<<"t "<<getDateAndTime()<<endl;
	gm.convertToSAT();
		SATInstance xInstance;
...
//	
	...
	for(int i=0;i<num_samples;i++)
	{
		...
		if (i > 1 && i % interval == 0){
			cout<<"Purging \n";
			purgeORStoreBayesMarginals(gm,other_gm,helper,store,sampling_order,rem_order,all_samples,rem_samples,all_weights,marginals_lb,marginals_ub,other_marginals,random,out);
			//purgeORStoreMarginals(gm,helper,store,sampling_order,rem_order,all_samples,rem_samples,all_weights,marginals_lb,marginals_ub,out);
			//purgeORStore(gm, helper, store, sampling_order, all_samples, all_weights,out);
			//weight_lb+=tmp.first;
			//weight_ub+=tmp.second;
...
void RBSampleSearch::computeBeliefs(GM& gm, JG& jg, int p_bound, vector<set<int> >& clusters,vector<int>& order,vector<int>& sampling_order, int num_samples)
{
	gm.convertToSAT();
	...
	for(int i=0;i<order.size();i++)
		...
		{
			helper_order.push_back(order[i]);
			other_variables.push_back(order[i]);
		}
		else
			helper_sampling_order.push_back(order[i]);
	//for(int i=(int)sampling_order.size()-1;i>-1;i--)
	...
	for(int i=0;i<num_samples;i++)
	{
		...
		vector<Function>& marginals_=jt.marginals;
		// Update marginals
		for(int j=0;j<sampling_order.size();j++)
		{
			int id=sampling_order[j];
			marginals[id][gm.variables[id]->value()]+=weight;
		}
		for(int j=0;j<other_variables.size();j++)
		{
			int id=other_variables[j];
			for(int k=0;k<gm.variables[id]->domain_size();k++)
			{
				marginals[id][k]+=marginals_[id].table()[k];
			}
		}
		for(int j=0;j<gm.variables.size();j++)
		{
...
pair<double,double> purgeORStoreSAT(GM& gm, IJGPSamplingHelper& helper, ORStore& store, vector<int>& sampling_order, vector<vector<int> >& all_samples, vector<double>& all_weights,ofstream& out)
{
...
RBSampleSearchSAT::RBSampleSearchSAT(): interval(10),outfilename("out")
{
}

Double RBSampleSearchSAT::computePEApp(GM&gm, JG& jg, const char* satfile, int p_bound, vector<int>& order,vector<int>& sampling_order, int num_samples, int max_restarts)
{
	// Initialize the SAT solver
    	/*        SATInstance xInstance;
		SATSolver xSATSolver(&xInstance);
...
pair<double,double> purgeORStoreSATSampling(GM& gm, IJGPSamplingHelper& helper, ORStore& store, vector<int>& sampling_order, vector<vector<int> >& all_samples, vector<vector<bool> >& full_samples, vector<double>& all_weights,ofstream& out)
{
...
void RBSampleSearchSAT::generateSamples(GM&gm, JG& jg, const char* satfile, int p_bound, vector<int>& order,vector<int>& sampling_order, int num_samples, int max_restarts)
{
	// Initialize the SAT solver
	Solver solver;
	gzFile in = gzopen(satfile, "rb");
